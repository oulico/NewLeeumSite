...
- 제작시 중점사항
  - 계획의 정확한 구현 - 자기역할을 잘하는 일꾼이 되도록
    - 철저한 와이어프레이밍을 통해 요구사항 정립
    - 제작을 염두에 두고 Html 구조를 계획
  - 읽기 쉬운 코드 - 쉬운 협업, 그리고 나자신을 위하여
    - 코드 컨벤션 정립 후 최대한 이행
    - DRY 원칙을 최대한 이행
    - "클린코드"에 입각한 변수명 작성
  - 사용성에 대한 고민 - 프론트엔드는 기술과 사람이 만나는 접점
    - 미디어쿼리를 통해 다양한 스크린환경 수용
    - QHD 화면에서도 같은 경험을 할 수 있도록 상대단위사용
    - "사용자를 생각하게 하지마"에 입각한 티켓 구매창 설계
  - 나가며
    - 완벽한 코드는 없다. - 앞으로도 리팩토링을 통해 다음과 같은 사항을 개선할 생각입니다.
      - 작업 후반, 미디어 쿼리 작성시 중복된 코드사용을 개선
      - 자바스크립트 코드의 모듈화하여 서로 영향을 주지 않도록 하기
      - 기능별로 코드를 "문단화"하여 정리하기
...
1. 철저한 와이어프레이밍
   ...
   <img src="../assets/img/README/image-20220721092601311.png" alt="image-20220721092601311" style="width: 600px;" />
...
프론트엔드는 프로젝트의 얼굴입니다. 제공하는 서비스가 아무리 좋더라도 이를 조작하는 부분이 불편하다면 사용자는 실망하고 더이상 이용을 하지 않게 됩니다. 이는 팀원들과 회사에 누가 되는 일이며, 때문에 프론트엔드 개발자는 막중한 책임감을 지녀야 한다고 생각합니다.
...
![scroll](../assets/img/README/scroll.gif)

1. 미디어 쿼리를 통한 스크린의 다양화
...
2. 티켓 창
   ...
   무엇보다 "사용자를 생각하게 하지마"라는 책을 읽고 사용성을 가장 많이 고민한 부분은 티켓 구매 페이지입니다.
   ...
   2. 사용자는 작동방식을 이해하려 하지 않는다.
      ...
      ![tickets](../assets/img/README/tickets.gif)
